...
dryclean <- R6::R6Class("dryclean",
  ...
  public = list(
    ...
    initialize = function(normal_vector = c(), pon_path, cov_path) {
      ...
      if(length(normal_vector) == 0 & !file.exists(pon_path)){                # if no normal samples passed AND pon does not exist
      ...
      if(length(normal_vector) != 0 ){  
        ...
        if (file.exists(normal_vector[1])){                                   # if normal vector is non-empty and the first normal sample exists, create a normal table
      ...
      if (!is.null(private$normal.table) & file.exists(private$pon.path)) {
        message(paste0("WARNING: If you create a new PON, it will be saved at ", private$pon.path))
      }

      private$status = "The dryclean class object is created. Recommended next step: "
      
      if(!is.null(private$normal.table)){
    ...
    prepare_detergent = function(use.all = TRUE, choose.randomly = FALSE, choose.by.clustering = FALSE, number.of.samples = 50, verbose = TRUE, num.cores = 1, tolerance = 0.0001, is.human = TRUE, build = "hg19", field = "reads.corrected", PAR.file = NULL, balance = TRUE, infer.germline = FALSE, signal.thresh = 0.3, pct.thresh = 0.80, wgs = TRUE, target_resolution = 1000, nochr = TRUE, all.chr = c(as.character(1:22), "X")){
      
      normal.table = private$normal.table
      
      if (length(normal.table) != 2){
      ...
      if (verbose){
        message("Finished making the PON and saving it to the provided path")
      }
    ...
    start_wash_cycle = function(centered = TRUE, mc.cores = 1, verbose = TRUE, whole_genome = TRUE, use.blacklist = FALSE, chr = NA, germline.filter = FALSE, germline.file = NA, field = "reads.corrected", is.human = TRUE, all.chr = c(as.character(1:22), "X"), testing = FALSE){
      ...
      pon.length <- rpca.1$template %>%
        gr2dt() %>%
        dplyr::filter(seqnames != "Y") %>%
        dt2gr() %>%
        length()
      pon.binsize = median(gr2dt(rpca.1$template)$width)
      
       if (tumor.length != pon.length & testing == FALSE) {
         stop("ERROR: seqlengths of coverage and PON do not match")
       }
    ...
    #'
    #' @return PON
    get_pon = function(){return(readRDS(private$pon.path))},
